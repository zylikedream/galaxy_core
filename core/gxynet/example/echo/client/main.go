...
func (e *EchoEventHandler) OnOpen(ctx context.Context, conn endpoint.Conn) error {
	gxylog.Infof("conn open, addr=%s", conn.Conn().RemoteAddr())
	go run(conn)
	return nil
}

func (e *EchoEventHandler) OnClose(ctx context.Context, conn endpoint.Conn) {
