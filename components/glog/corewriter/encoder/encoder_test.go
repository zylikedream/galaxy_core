...
func TestHook(t *testing.T) {
	...
	[log]
		level_encoder = "capital"
	[log.encoder_config] # 完整字段参考github.com/zylikedream/galaxy/components/glog/corewriter/encoder/encoder.go的config
		message_key = "msg1"
		level_key = "level1"
	...
	econfig, err := newZapEncoderConfig(configure.WithParent("log"))
	if err != nil {
		t.Error(err)
		return
	}
